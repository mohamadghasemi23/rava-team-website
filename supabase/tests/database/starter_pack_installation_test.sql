...
select throws_ok(
  $$select public.install_starter_pack((select id from public.sites where slug='install-site-b'),(select v.id from public.starter_content_pack_versions v join public.starter_content_packs p on p.id=v.starter_pack_id where p.key='services.digital-agency.rava-team' and v.version=1),(select v.id from public.template_versions v join public.template_catalog t on t.id=v.template_id where t.key='rava-commerce-modern' and v.version=1),'ebbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb',array['fa'],'{}')$$,
  '22023','incompatible_template','service content cannot install with a commerce Template'
);
