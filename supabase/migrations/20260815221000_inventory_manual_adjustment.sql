...
begin
 ...
 insert into public.inventory_balances(tenant_id,location_id,product_id,variant_id,on_hand,reserved,reorder_point) values(p_tenant,p_location,product_id,p_variant,0,0,0) on conflict(tenant_id,location_id,variant_id) do nothing;
 ...
 insert into public.inventory_ledger(tenant_id,location_id,product_id,variant_id,movement_type,quantity_delta,reason,reference_type,reference_id,created_by) values(p_tenant,p_location,product_id,p_variant,'manual_adjustment',p_delta,left(trim(p_reason),500),left(coalesce(p_reference_type,'manual'),80),p_reference_id,p_actor);
 insert into public.inventory_manual_adjustments(tenant_id,location_id,variant_id,quantity_delta,reason,reference_type,reference_id,actor_user_id) values(p_tenant,p_location,p_variant,p_delta,left(trim(p_reason),500),left(coalesce(p_reference_type,'manual'),80),p_reference_id,p_actor);
 return new_on_hand;
end$$;
revoke all on function public.manual_adjust_inventory(uuid,uuid,uuid,integer,text,uuid,text,uuid) from public,anon;grant execute on function public.manual_adjust_inventory(uuid,uuid,uuid,integer,text,uuid,text,uuid) to authenticated;
...
create or replace function public.release_order_inventory_manual(p_tenant uuid,p_order uuid,p_reason text,p_actor uuid default null) returns integer language plpgsql security definer set search_path=public as $$declare r record;n integer:=0;begin
 if not public.can_access_tenant(p_tenant,null) then raise exception 'forbidden';end if;if length(trim(coalesce(p_reason,'')))<3 then raise exception 'reason_required';end if;
 for r in select * from public.order_inventory_reservations where tenant_id=p_tenant and order_id=p_order and status='active' for update loop update public.order_inventory_reservations set status='released' where id=r.id; n:=n+1;end loop;
 insert into public.event_logs(tenant_id,category,event_name,actor_user_id,summary_fa,metadata) values(p_tenant,'audit','inventory.order_reservation.manual_release',p_actor,'رزرو موجودی سفارش به‌صورت دستی آزاد شد.',jsonb_build_object('order_id',p_order,'reason',left(trim(p_reason),500),'released_rows',n));return n;end$$;
revoke all on function public.release_order_inventory_manual(uuid,uuid,text,uuid) from public,anon;grant execute on function public.release_order_inventory_manual(uuid,uuid,text,uuid) to authenticated;
