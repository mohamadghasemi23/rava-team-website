-- Contact submissions use a narrowly-scoped public RPC instead of a Netlify service-role key.
-- Provision CONTACT_RPC_GATEWAY_TOKEN in the server environment and store only its SHA-256
-- hash in private.contact_gateway_config. Never commit the raw token.

create schema if not exists private;
revoke all on schema private from public, anon, authenticated;

create table if not exists private.contact_gateway_config (
  singleton boolean primary key default true check (singleton),
  token_hash text not null check (length(token_hash)=64),
  updated_at timestamptz not null default now()
);
revoke all on private.contact_gateway_config from public, anon, authenticated;
...
begin
  ...
  values('contact',client_key,window_start,1)
