...
begin
  ...
  select p_tenant,p.id,p_day,
    ...
    coalesce((select sum(oi.line_total) from public.order_items oi join public.orders o on o.id=oi.order_id where o.tenant_id=p_tenant and oi.product_id=p.id and o.created_at>=p_day::timestamptz and o.created_at<(p_day+1)::timestamptz and o.status not in('cancelled','refunded')),0),
    0,
  ...
  select p_tenant,p_day,
    ...
    coalesce((select sum(total) from public.orders where tenant_id=p_tenant and payment_status='paid' and paid_at>=p_day::timestamptz and paid_at<(p_day+1)::timestamptz),0),
