-- Netlify intentionally withholds secret environment variables from Deploy Preview.
-- A separate, narrowly-scoped preview gateway hash may be provisioned for QA.
-- Never commit the raw preview or production gateway token.

alter table private.contact_gateway_config
  add column if not exists preview_token_hash text
  check (preview_token_hash is null or length(preview_token_hash)=64);
...
begin
  select token_hash, preview_token_hash into expected_hash, preview_hash
  from private.contact_gateway_config where singleton=true;
  supplied_hash := case when gateway_token='' then '' else encode(extensions.digest(gateway_token,'sha256'),'hex') end;
  if supplied_hash='' or (supplied_hash is distinct from expected_hash and supplied_hash is distinct from preview_hash) then
...
-- Provision preview_token_hash separately per environment after generating the preview token.
