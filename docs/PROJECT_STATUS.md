...
The post-deploy owner walkthrough found that Living System was absent from Template selection. Repository and live Staging evidence confirmed the renderer existed but no `template_catalog`, published `template_versions` or starter-pack compatibility migration had been created. Migration `20260902134253_rava_living_system_template.sql` and six pgTAP assertions now exist in the working branch; a real Staging PostgreSQL transaction applied all statements successfully and rolled them back. The migration is not yet applied persistently and the Template remains absent until CI passes, a fresh backup succeeds and the migration/test pair passes on Staging.
...
1. Pass CI for the Living System catalog migration, create a fresh Staging PostgreSQL backup, apply the migration and run its six pgTAP assertions; then confirm the Template appears and can create a draft preview without publishing.
2. Obtain fresh real-browser desktop/mobile/open-menu captures for the separate implementation-acceptance review.
