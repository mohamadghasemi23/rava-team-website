...
An unvalidated implementation pass now replaces the Living System Hero's legacy bitmap product plates with localized HTML/CSS product-console primitives and adds an actual selectable four-stage journey. Selecting content, design, preview or publish changes the main and responsive product views; hover/focus provides a restrained activation seam, while reduced-motion rules keep the static result complete. Mobile uses a single-column, touch-oriented console and hides the desktop editor rail. Static inspection confirms the component no longer references the obsolete editor/responsive bitmap assets. This pass is not accepted, committed as release-ready or deployable until feature-standard, RPIM, TypeScript, production Build and real-browser desktop/mobile/menu/interaction Gates pass; the current Codex environment has no Node runtime and cannot access Docker.
