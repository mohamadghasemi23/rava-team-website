...
The current V4 Hero visual authority is the owner-approved mountain-console composition preserved in `.impeccable/mocks/rava-flagship-v4-hero-desktop-rest-approved.png`, `.impeccable/mocks/rava-flagship-v4-hero-desktop-design-approved.png` and `.impeccable/mocks/rava-flagship-v4-hero-mobile-rest-approved.png`. It uses one monolithic RAVA product display, a restrained four-stage interaction, a soft contact shadow and the concave pearl-to-navy transition. Implementation commit `84cd3da` replaces V3's separate editor/tablet/phone composition with localized, performance-budgeted desktop/mobile WebP renders plus semantic keyboard/touch hotspots and a reduced-motion-safe active signal. It is pushed and all GitHub Gates pass, but it is not yet real-browser-captured, owner-accepted or deployed. Production, DNS and `ravateam.ir` remain unchanged.
...
1. Capture and compare the coded V4 Hero with the approved desktop/mobile/rest/active references; obtain explicit implementation acceptance before any Staging deployment.
2. If the comparison passes, request explicit approval for an isolated Staging deployment of the exact immutable V4 image.
