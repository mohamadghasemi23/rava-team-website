...
A replacement implementation in commit `19af1da` imports the new single-authority `rava-living-system-v2.module.css` stylesheet. It contains one coherent desktop/mobile/navigation/product/Journey system and no URL or reference to the rejected lamp, stone, cup or legacy product bitmaps. GitHub and isolated Staging Gates passed, but the owner's subsequent 1240px capture is visually rejected and `19af1da` is not implementation-accepted. Independent reviews found concrete causes: Hero regions overlap, the release rail is clipped, the approved RAVA dashboard became an abstract form, Journey stages became repeated skeletons and CTA labels are not visibly rendered. The earlier font-missing hypothesis was disproved: the Fontsource variable packages are installed and imported globally. Four replacement V3 desktop/mobile/closed/open-menu previews were explicitly approved on 2026-09-03. The implementation worktree now uses a single rewritten stylesheet, a semantic RAVA dashboard, distinct Journey stages, protected 1240px geometry, an in-bounds release rail and a brand-aligned final CTA. It is not yet committed, CI-validated, browser-captured or implementation-accepted. Production, DNS and `ravateam.ir` remain unchanged.
...
1. Validate and commit the V3 implementation, then run the complete GitHub Gate set without deploying it.
2. Capture and compare the coded result with the four approved V3 references; obtain explicit implementation acceptance before any Staging deployment.
