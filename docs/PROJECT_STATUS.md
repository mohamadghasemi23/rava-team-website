...
- **Last confirmed product/design commit pushed to the GitHub branch:** `b721072` (`design: implement approved flagship v3 system`)
...
The current V4 Hero visual authority is the owner-approved mountain-console composition preserved in `.impeccable/mocks/rava-flagship-v4-hero-desktop-rest-approved.png`, `.impeccable/mocks/rava-flagship-v4-hero-desktop-design-approved.png` and `.impeccable/mocks/rava-flagship-v4-hero-mobile-rest-approved.png`. It uses one monolithic RAVA product display, a restrained four-stage interaction, a soft contact shadow and the concave pearl-to-navy transition. Local commit `84cd3da` replaces V3's separate editor/tablet/phone composition with localized, performance-budgeted desktop/mobile WebP renders plus semantic keyboard/touch hotspots and a reduced-motion-safe active signal. It is not yet pushed, CI-validated, browser-captured, owner-accepted or deployed. Production, DNS and `ravateam.ir` remain unchanged.
...
1. Validate and commit the V4 Hero implementation, then run the complete GitHub Gate set without deploying it.
2. Capture and compare the coded result with the approved V4 desktop/mobile/rest/active references; obtain explicit implementation acceptance before any Staging deployment.
