...
export default function RavaCommercialHomepage({payload,navigation,previewBasePath}:Props){
  ...
  const setDragProgress=useCallback((progress:number)=>{if(!emblaApi||window.matchMedia('(prefers-reduced-motion: reduce)').matches)return;const selected=emblaApi.selectedScrollSnap();const mobile=emblaApi.rootNode().clientWidth<=820;const maxAngle=mobile?4.5:7.5;emblaApi.slideNodes().forEach((node,index)=>{const card=node.firstElementChild as HTMLElement|null;if(!card)return;const activeCard=index===selected;const amount=activeCard?Math.abs(progress):Math.max(0,Math.abs(progress)-.1);const normalized=Math.min(amount/.32,1);const direction=(Math.sign(progress)||1)*(activeCard?1:-1);card.style.setProperty('--drag-rotate',`${direction*normalized*maxAngle}deg`);card.style.setProperty('--drag-scale',String(1-normalized*(activeCard?(mobile?.025:.035):.02)));card.style.setProperty('--drag-depth',`${activeCard?normalized*(mobile?10:18):-normalized*10}px`);card.style.setProperty('--drag-shadow-x',`${-direction*normalized*(mobile?10:18)}px`);card.style.setProperty('--drag-parallax',`${direction*normalized*(mobile?7:12)}px`);card.style.setProperty('--drag-edge-opacity',String(activeCard?normalized*.78:normalized*.24));card.style.setProperty('--drag-edge-angle',direction>0?'270deg':'90deg')})},[emblaApi])
  const clearDragProgress=useCallback(()=>{pointerStart.current=null;emblaApi?.rootNode().removeAttribute('data-dragging');emblaApi?.slideNodes().forEach(node=>{const card=node.firstElementChild as HTMLElement|null;['--drag-rotate','--drag-scale','--drag-depth','--drag-shadow-x','--drag-parallax','--drag-edge-opacity','--drag-edge-angle'].forEach(name=>card?.style.removeProperty(name))})},[emblaApi])
  useEffect(()=>{
    if(!emblaApi)return
  ...
  return <div className={styles.page} dir={isFa?'rtl':'ltr'} lang={isFa?'fa':'en'}>
    ...
    <main id="main">
      ...
      <section className={styles.showcase} id="showcase" aria-labelledby="showcase-title" onMouseEnter={()=>setInteractionPaused(true)} onMouseLeave={()=>setInteractionPaused(false)} onFocusCapture={()=>setInteractionPaused(true)} onBlurCapture={event=>{if(!event.currentTarget.contains(event.relatedTarget as Node))setInteractionPaused(false)}}>
        ...
        <div className={styles.viewport} ref={emblaRef} onPointerDown={event=>{event.currentTarget.setPointerCapture(event.pointerId);event.currentTarget.dataset.dragging='true';pointerStart.current=event.clientX;setInteractionPaused(true)}} onPointerMove={event=>{if(pointerStart.current===null)return;const progress=Math.max(-.32,Math.min(.32,(event.clientX-pointerStart.current)/Math.max(event.currentTarget.clientWidth,1)));setDragProgress(progress)}} onPointerUp={event=>{event.currentTarget.releasePointerCapture(event.pointerId);clearDragProgress();setInteractionPaused(false);autoplay.current.reset()}} onPointerCancel={()=>{clearDragProgress();setInteractionPaused(false)}} onLostPointerCapture={()=>{clearDragProgress();setInteractionPaused(false)}}>
