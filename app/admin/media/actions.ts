...
async function operationalFailure(error:unknown,scope:Scope|undefined,eventType:string,message:string):Promise<MediaActionResult>{
  ...
  return{ok:false,message:errorId?`${message} شناسه خطا: ${errorId}`:message,errorId}
}

export async function uploadMedia(formData:FormData):Promise<MediaActionResult>{
  const current=await actor();if(!current)return{ok:false,message:'نشست کاربری معتبر نیست.'}
  const siteId=String(formData.get('site_id')??'');const scope=await authorizeSite(current.supabase,siteId);if(!scope)return{ok:false,message:'سایت پیدا نشد یا اجازه مدیریت رسانه آن را ندارید.'}
  const value=formData.get('file');if(!(value instanceof File)||value.size<1||value.size>MAX_BYTES)return{ok:false,message:'فایل باید تصویری و حداکثر ۱۰ مگابایت باشد.'}
  if(value.name.length>180)return{ok:false,message:'نام فایل بیش از حد طولانی است.'}
  const alt=String(formData.get('alt_text')??'').trim();if(alt.length>300)return{ok:false,message:'Alt Text باید حداکثر ۳۰۰ نویسه باشد.'}
  const bytes=new Uint8Array(await value.arrayBuffer());const mime=detectedMime(bytes)
  if(!mime||value.type!==mime||!MIME_EXTENSIONS[mime])return{ok:false,message:'فرمت واقعی فایل معتبر نیست. فقط JPEG، PNG، WebP و GIF مجاز است.'}
  ...
  try{
    ...
    revalidatePath('/admin/media');return{ok:true,message:'تصویر با موفقیت آپلود شد.',asset:data as MediaAsset}
  }catch(error){return operationalFailure(error,scope,'media.asset.upload_failed','آپلود تصویر انجام نشد.')}
}
...
export async function updateMediaAlt(id:string,value:string):Promise<MediaActionResult>{
  const alt=value.trim();if(!UUID_RE.test(id)||alt.length>300)return{ok:false,message:'شناسه یا Alt Text معتبر نیست.'}
  const scoped=await assetScope(id);if(!scoped)return{ok:false,message:'تصویر پیدا نشد یا اجازه ویرایش آن را ندارید.'}
  ...
  if(error||!data)return operationalFailure(error,scoped.scope,'media.asset.alt_update_failed','ویرایش Alt Text انجام نشد.')
  ...
  revalidatePath('/admin/media');return{ok:true,message:'Alt Text با موفقیت ذخیره شد.'}
}

export async function deleteMedia(id:string):Promise<MediaActionResult>{
  const scoped=await assetScope(id);if(!scoped)return{ok:false,message:'تصویر پیدا نشد یا اجازه حذف آن را ندارید.'}
  ...
  if(marked.error||!marked.data)return operationalFailure(marked.error,scoped.scope,'media.asset.delete_failed','حذف تصویر انجام نشد.')
  const removed=await scoped.supabase.storage.from('rava-media').remove([scoped.asset.storage_path])
  if(removed.error){await scoped.supabase.from('media_assets').update({deleted_at:null}).eq('id',id).eq('site_id',scoped.scope.siteId);return operationalFailure(removed.error,scoped.scope,'media.asset.storage_delete_failed','حذف فایل از Storage انجام نشد.')}
  ...
  revalidatePath('/admin/media');return{ok:true,message:'تصویر با موفقیت حذف شد.'}
}
