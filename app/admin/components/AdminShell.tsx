...
const navigation: NavItem[] = [
  ...
  { label: 'پروژه‌ها', icon: '▦', keywords: ['projects', 'work', 'پروژه', 'نمونه کار'], disabled: true },
  { label: 'خدمات', icon: '◇', keywords: ['services', 'service', 'خدمات'], disabled: true },
  { label: 'درباره راوا', icon: '○', keywords: ['about', 'درباره', 'راوا'], disabled: true },
  { label: 'پیام‌ها', icon: '✉', keywords: ['messages', 'leads', 'contact', 'پیام', 'درخواست'], disabled: true },
...
export default function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="rava-admin-frame" dir="rtl">
    <button
      className="rava-admin-mobile-trigger"
      type="button"
    ...
    <aside className={`rava-admin-sidebar${mobileOpen ? ' is-open' : ''}`} aria-label="منوی مدیریت">
      <div className="rava-admin-brand-row">
        <Link className="rava-admin-brand" href="/admin">
          <b>RAVA</b> TEAM
          <small>مدیریت سایت راوا</small>
        </Link>
      ...
      <label className="rava-admin-search">
        <span>⌕</span>
        <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="جست‌وجو در پنل…" />
      </label>
      ...
      <nav className="rava-admin-tree">
        ...
        {filteredNavigation.map((item) => {
          if (item.href) {
            ...
            return (
              <Link key={item.label} href={item.href} className={`rava-admin-nav-item${active ? ' is-active' : ''}`}>
          ...
          return (
            <div key={item.label} className="rava-admin-nav-item is-disabled" title="در فاز اجرای V1 فعال می‌شود">
              <i>{item.icon}</i><span>{item.label}</span><small>V1</small>
            </div>
          )
        })}
      </nav>
