...
export async function createPage(_state: AdminActionState, formData: FormData): Promise<AdminActionState> {
  ...
  if (!title || !slug || !scope) return { ok:false, message:'عنوان، آدرس یا سایت انتخاب‌شده معتبر نیست.', nonce:Date.now() }
  if(!await authorize(scope,PERMISSIONS.CMS_MANAGE)) return {ok:false,message:'اجازه ساخت صفحه برای این سایت را ندارید.',nonce:Date.now()}
...
export async function updatePage(_state: AdminActionState, formData: FormData): Promise<AdminActionState> {
  ...
  const scoped=await pageScope(supabase,id);if(!scoped||!await authorize(scoped,PERMISSIONS.CMS_MANAGE)||status==='published'&&!await authorize(scoped,PERMISSIONS.CMS_PUBLISH))return{ok:false,message:'صفحه پیدا نشد یا اجازه این تغییر را ندارید.',nonce:Date.now()}
...
export async function setPageStatus(_state:AdminActionState,formData:FormData):Promise<AdminActionState>{
 const {supabase,userId}=await getActor();const id=String(formData.get('id')??'');const status=String(formData.get('status')??'');if(!id||!['draft','published','hidden'].includes(status))return{ok:false,message:'وضعیت انتخاب‌شده معتبر نیست.',nonce:Date.now()};const scoped=await pageScope(supabase,id);const permission=status==='published'?PERMISSIONS.CMS_PUBLISH:PERMISSIONS.CMS_MANAGE;if(!scoped||!await authorize(scoped,permission))return{ok:false,message:'صفحه پیدا نشد یا اجازه تغییر وضعیت آن را ندارید.',nonce:Date.now()};const{error}=await supabase.from('pages').update({status,published_at:status==='published'?new Date().toISOString():null,updated_by:userId,updated_at:new Date().toISOString()}).eq('id',id).eq('site_id',scoped.siteId);if(error)return{ok:false,message:errorMessage(error,'تغییر وضعیت صفحه انجام نشد.'),nonce:Date.now()};await recordAuditEvent({action:status==='published'?'cms.page.published':'cms.page.status_changed',entityType:'page',entityId:id,organizationId:scoped.organizationId,siteId:scoped.siteId,before:{status:scoped.page.status},after:{status},context:{source:'admin_pages'},severity:status==='published'?'notice':'info'});revalidatePath('/admin');revalidatePath('/admin/pages');return{ok:true,message:status==='published'?'صفحه با موفقیت منتشر شد.':status==='hidden'?'صفحه با موفقیت مخفی شد.':'صفحه به پیش‌نویس منتقل شد.',nonce:Date.now()}
}

export async function createBlock(_state:AdminActionState,formData:FormData):Promise<AdminActionState>{
 const {supabase}=await getActor();const pageId=String(formData.get('page_id')??'');const blockType=String(formData.get('block_type')??'text');const scoped=await pageScope(supabase,pageId);if(!scoped||!await authorize(scoped,PERMISSIONS.CMS_MANAGE))return{ok:false,message:'صفحه پیدا نشد یا اجازه ویرایش آن را ندارید.',nonce:Date.now()};
...
export async function updateBlock(_state:AdminActionState,formData:FormData):Promise<AdminActionState>{
 const {supabase}=await getActor();const id=String(formData.get('id')??'');const pageId=String(formData.get('page_id')??'');const blockType=String(formData.get('block_type')??'text');const scoped=await pageScope(supabase,pageId);if(!scoped||!UUID_RE.test(id)||!await authorize(scoped,PERMISSIONS.CMS_MANAGE))return{ok:false,message:'بلاک پیدا نشد یا اجازه ویرایش آن را ندارید.',nonce:Date.now()};let data:Record<string,unknown>={};
 ...
 const {error}=await supabase.from('page_blocks').update({data,updated_at:new Date().toISOString()}).eq('id',id).eq('page_id',pageId);if(error)return{ok:false,message:errorMessage(error,'ویرایش بلاک انجام نشد.'),nonce:Date.now()};revalidatePath(`/admin/pages/${pageId}`);return{ok:true,message:'تغییرات بلاک با موفقیت ذخیره شد.',nonce:Date.now()}
}

export async function toggleBlock(_state:AdminActionState,formData:FormData):Promise<AdminActionState>{
 const {supabase}=await getActor();const id=String(formData.get('id')??'');const pageId=String(formData.get('page_id')??'');const visible=String(formData.get('visible'))==='true';const scoped=await pageScope(supabase,pageId);if(!scoped||!UUID_RE.test(id)||!await authorize(scoped,PERMISSIONS.CMS_MANAGE))return{ok:false,message:'بلاک پیدا نشد یا اجازه تغییر آن را ندارید.',nonce:Date.now()};const{error}=await supabase.from('page_blocks').update({visible:!visible,updated_at:new Date().toISOString()}).eq('id',id).eq('page_id',pageId);if(error)return{ok:false,message:errorMessage(error,'تغییر نمایش بلاک انجام نشد.'),nonce:Date.now()};revalidatePath(`/admin/pages/${pageId}`);return{ok:true,message:visible?'بلاک مخفی شد.':'بلاک نمایش داده شد.',nonce:Date.now()}
}

export async function moveBlock(_state:AdminActionState,formData:FormData):Promise<AdminActionState>{
 const {supabase}=await getActor();const id=String(formData.get('id')??'');const pageId=String(formData.get('page_id')??'');const direction=String(formData.get('direction')??'up');const scoped=await pageScope(supabase,pageId);if(!scoped||!UUID_RE.test(id)||!await authorize(scoped,PERMISSIONS.CMS_MANAGE))return{ok:false,message:'بلاک پیدا نشد یا اجازه جابه‌جایی آن را ندارید.',nonce:Date.now()};const{data:blocks,error}=await supabase.from('page_blocks').select('id,position').eq('page_id',pageId).order('position',{ascending:true});if(error||!blocks)return{ok:false,message:errorMessage(error,'جابه‌جایی انجام نشد.'),nonce:Date.now()};const index=blocks.findIndex(b=>b.id===id);const target=direction==='up'?index-1:index+1;if(index<0||target<0||target>=blocks.length)return{ok:false,message:'این بلاک بیشتر از این قابل جابه‌جایی نیست.',nonce:Date.now()};const first=await supabase.from('page_blocks').update({position:blocks[target].position}).eq('id',blocks[index].id).eq('page_id',pageId);const second=await supabase.from('page_blocks').update({position:blocks[index].position}).eq('id',blocks[target].id).eq('page_id',pageId);if(first.error||second.error)return{ok:false,message:'جابه‌جایی بلاک کامل نشد.',nonce:Date.now()};revalidatePath(`/admin/pages/${pageId}`);return{ok:true,message:'ترتیب بلاک با موفقیت تغییر کرد.',nonce:Date.now()}
}

export async function deleteBlock(_state:AdminActionState,formData:FormData):Promise<AdminActionState>{const{supabase}=await getActor();const id=String(formData.get('id')??'');const pageId=String(formData.get('page_id')??'');const scoped=await pageScope(supabase,pageId);if(!scoped||!UUID_RE.test(id)||!await authorize(scoped,PERMISSIONS.CMS_MANAGE))return{ok:false,message:'بلاک پیدا نشد یا اجازه حذف آن را ندارید.',nonce:Date.now()};const{error}=await supabase.from('page_blocks').delete().eq('id',id).eq('page_id',pageId);if(error)return{ok:false,message:errorMessage(error,'حذف بلاک انجام نشد.'),nonce:Date.now()};revalidatePath(`/admin/pages/${pageId}`);return{ok:true,message:'بلاک با موفقیت حذف شد.',nonce:Date.now()}}

export async function deletePage(_state:AdminActionState,formData:FormData):Promise<AdminActionState>{const{supabase}=await getActor();const id=String(formData.get('id')??'');const scoped=await pageScope(supabase,id);if(!scoped||!await authorize(scoped,PERMISSIONS.CMS_MANAGE))return{ok:false,message:'صفحه پیدا نشد یا اجازه حذف آن را ندارید.',nonce:Date.now()};const{error:blocksError}=await supabase.from('page_blocks').delete().eq('page_id',id);if(blocksError)return{ok:false,message:errorMessage(blocksError,'حذف محتوای صفحه انجام نشد.'),nonce:Date.now()};const{error}=await supabase.from('pages').delete().eq('id',id).eq('site_id',scoped.siteId);if(error)return{ok:false,message:errorMessage(error,'حذف صفحه انجام نشد.'),nonce:Date.now()};await recordAuditEvent({action:'cms.page.deleted',entityType:'page',entityId:id,organizationId:scoped.organizationId,siteId:scoped.siteId,before:{title:scoped.page.title,slug:scoped.page.slug,status:scoped.page.status},context:{source:'admin_pages'},severity:'notice'});revalidatePath('/admin');revalidatePath('/admin/pages');return{ok:true,message:`صفحه «${scoped.page.title}» با موفقیت حذف شد.`,redirectTo:`/admin/pages?site=${scoped.siteId}`,nonce:Date.now()}}
