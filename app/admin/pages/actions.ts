...
export async function updatePage(_state: AdminActionState, formData: FormData): Promise<AdminActionState> {
  ...
  if(!id||!title||!slug||!['draft','published','hidden','scheduled'].includes(status)) return {ok:false,message:l('اطلاعات فرم کامل یا معتبر نیست.','The form information is incomplete or invalid.'),nonce:Date.now()}
