...
export default async function PagesAdminPage({searchParams}:{searchParams:Promise<{site?:string}>}) {
  ...
  return <main className="admin-shell">
    <header className="admin-head"><div><span>{l('مرکز مدیریت راوا','RAVA CONTROL CENTER')}</span><h1>{l(`صفحه‌های ${site.name}`,`${site.name} pages`)}</h1><p>{l('سایت فعال:','Active site:')} <b>{site.name}</b> · <span dir="ltr">{site.slug}</span></p></div><div className="admin-actions"><Link className="admin-link" href="/admin/pages">{l('تغییر سایت','Change site')}</Link><Link className="admin-link" href={`/admin/platform/sites/${site.id}`}>{l('تنظیمات سایت','Site settings')}</Link></div></header>
