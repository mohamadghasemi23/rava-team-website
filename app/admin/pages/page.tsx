...
const STRUCTURE=['home','services','portfolio','about','process','faq','contact','privacy','service-detail']

export default async function PagesAdminPage({searchParams}:{searchParams:Promise<{site?:string;locale?:string;sort?:string}>}){
 ...
 const{site:requestedSite,locale:localeParam,sort:sortParam}=await searchParams
 ...
 const[canManage,canPublish,{data},{data:homeMappings}]=await Promise.all([
  ...
  supabase.from('pages').select('id,title,slug,status,created_at,updated_at').eq('site_id',site.id),
  supabase.from('site_home_pages').select('locale,page_id').eq('site_id',site.id),
 ])
 const pages=data??[],activeLocale=['fa','en','all'].includes(localeParam??'')?localeParam!:(site.primary_locale==='en'?'en':'fa'),activeSort=['structure','created','updated'].includes(sortParam??'')?sortParam!:'structure'
 const pageLocale=(slug:string)=>slug.endsWith('-en')?'en':'fa',baseSlug=(slug:string)=>slug.replace(/-en$/,'')
 const rank=(slug:string)=>{const index=STRUCTURE.indexOf(baseSlug(slug));return index<0?999:index}
 const visiblePages=pages.filter(page=>activeLocale==='all'||pageLocale(page.slug)===activeLocale).sort((a,b)=>activeSort==='created'?new Date(b.created_at).getTime()-new Date(a.created_at).getTime()||a.id.localeCompare(b.id):activeSort==='updated'?new Date(b.updated_at).getTime()-new Date(a.updated_at).getTime()||a.id.localeCompare(b.id):rank(a.slug)-rank(b.slug)||a.title.localeCompare(b.title,locale==='fa'?'fa':'en'))
 const listHref=(nextLocale:string,nextSort:string)=>`/admin/pages?site=${site.id}&locale=${nextLocale}&sort=${nextSort}`
 return <main className="admin-shell rava-pages-simple">
  ...
  <section className="rava-pages-list"><div className="rava-pages-list-head"><div><h2>{l('صفحه‌ها','Pages')}</h2><p>{visiblePages.length} {l('صفحه · نمایش با ترتیب انتخاب‌شده','pages · ordered by your selection')}</p></div><div className="rava-pages-tools"><nav aria-label={l('زبان صفحه‌ها','Page language')}>{[['fa',l('فارسی','Persian')],['en','English'],['all',l('همه','All')]].map(([key,label])=><Link className={activeLocale===key?'is-active':''} href={listHref(key,activeSort)} key={key}>{label}</Link>)}</nav><div className="rava-pages-sort">{[['structure',l('ترتیب سایت','Site order')],['created',l('جدیدترین','Newest')],['updated',l('آخرین ویرایش','Last edited')]].map(([key,label])=><Link className={activeSort===key?'is-active':''} href={listHref(activeLocale,key)} key={key}>{label}</Link>)}</div></div></div>
   {visiblePages.length===0?<div className="admin-empty">{l('در این زبان هنوز صفحه‌ای ساخته نشده است.','No pages exist in this language yet.')}</div>:<div className="rava-page-rows">{visiblePages.map((page,index)=>{const published=page.status==='published';return <article className="rava-page-row" key={page.id}><span className="rava-page-index">{new Intl.NumberFormat(locale==='fa'?'fa-IR':'en-GB',{minimumIntegerDigits:2}).format(index+1)}</span><div className="rava-page-identity"><span className="rava-page-sheet"><AdminIcon name="pages" size={18}/></span><div><b>{page.title}</b><small dir="ltr">/{page.slug}</small></div></div><div className="rava-page-row-actions"><span className={`status-pill status-${page.status}`}>{statusLabel(page.status)}</span>{canManage?<Link className="rava-page-edit" href={`/admin/pages/${page.id}`}>{l('ویرایش','Edit')}</Link>:null}{canManage||canPublish?<details className="rava-page-more"><summary aria-label={l('عملیات بیشتر','More actions')}>•••</summary><div>{(published?canManage:canPublish)?<ActionForm action={setPageStatus} confirmTitle={published?l('مخفی‌کردن صفحه','Hide page'):l('انتشار صفحه','Publish page')} confirmMessage={published?l(`صفحه «${page.title}» مخفی شود؟`,`Hide “${page.title}”?`):l(`صفحه «${page.title}» منتشر شود؟`,`Publish “${page.title}”?`)} confirmLabel={l('بله، انجام شود','Yes, continue')}><input type="hidden" name="id" value={page.id}/><input type="hidden" name="status" value={published?'hidden':'published'}/><button type="submit">{published?l('مخفی‌کردن','Hide'):l('انتشار','Publish')}</button></ActionForm>:null}{canManage?<ActionForm action={deletePage} danger confirmTitle={l('حذف کامل صفحه','Delete page')} confirmMessage={l(`صفحه «${page.title}» و محتوای آن حذف شود؟`,`Delete “${page.title}” and its content?`)} confirmLabel={l('بله، حذف شود','Yes, delete')}><input type="hidden" name="id" value={page.id}/><button className="is-danger" type="submit">{l('حذف صفحه','Delete page')}</button></ActionForm>:null}</div></details>:null}</div></article>})}</div>}
  </section>
 </main>
}
