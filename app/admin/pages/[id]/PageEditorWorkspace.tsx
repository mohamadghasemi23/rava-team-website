...
type Props={locale:'fa'|'en';site:{id:string;name:string};page:{id:string;title:string;status:string};blocks:Block[];demoPreviewPath?:string;identity?:EditorIdentity;issues?:EditorIssue[]}
...
export default function PageEditorWorkspace({locale,site,page,blocks,demoPreviewPath,identity,issues=[]}:Props){
 ...
 return <section className={`${styles.workspace} ${fullPreview?styles.fullPreview:''}`} aria-label={l('ویرایشگر صفحه','Page editor')}>
  {identity?<AdminContextHeader locale={locale} pageTitle={page.title} site={site} template={{name:identity.templateName,href:identity.templateHref}} identity={identity} issues={issues} onHelp={()=>setHelp(value=>!value)}/>:null}
