...
import {generateSeoWithOpenAI} from '@/lib/ai/providers/openai-responses'
...
export async function requestSeoSuggestion(pageId:string):Promise<SeoSuggestionState>{
 ...
 catch(error){await recordErrorEvent({error,category:'ai_provider',eventType:'seo.ai.suggestion_failed',publicMessage:l('ساخت پیشنهاد هوشمند انجام نشد.','The AI suggestion could not be generated.'),organizationId:site.organization_id,siteId:site.id,route:`/admin/pages/${pageId}`,context:{provider:'openai'},severity:'warning'});return{ok:false,message:l('پیشنهاد هوشمند آماده نشد. تنظیمات اتصال یا سرویس را بررسی کنید.','The AI suggestion is unavailable. Check the provider configuration or service.')}}
}
