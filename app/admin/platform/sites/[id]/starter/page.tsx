...
export default async function StarterPage({params}:{params:Promise<{id:string}>}){
 ...
 const{data:site}=await supabase.from('sites').select('id,name,slug,primary_locale').eq('id',id).maybeSingle();if(!site)notFound()
