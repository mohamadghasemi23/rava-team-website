  ...
  return <section className="starter-wizard" dir={locale==='fa'?'rtl':'ltr'}>
    ...
    {!option?<div className="admin-empty">{t.empty}</div>:<div className="starter-grid">
      <div className="starter-controls">
        ...
        <div className="starter-card"><b>{locale==='fa'?option.industry.name_fa:option.industry.name_en}</b><p>{locale==='fa'?option.pack.description_fa:option.pack.description_en}</p><small>{locale==='fa'?`بسته نسخه ${option.pack.version}`:`${option.pack.commercial_tier.toUpperCase()} · Pack v${option.pack.version}`}</small></div>
      ...
      <div className="starter-preview"><div><span>{t.preview}</span><b>{pages.length} {t.pages}</b></div><h3>{locale==='fa'?option.template.name_fa:option.template.name_en} · {locale==='fa'?'نسخه':'v'} {option.template.version}</h3><p>{locale==='fa'?option.template.description_fa:option.template.description_en}</p><div className="starter-page-list">{pages.map((page,index)=><article key={`${page.slug}:${index}`}><span>{String(index+1).padStart(2,'0')}</span><div><b>{page.title}</b><small dir="ltr">/{page.slug}</small></div><em>{locale==='fa'?'پیش‌نویس':'DRAFT'}</em></article>)}</div></div>
    </div>}
  </section>
}
