...
export default async function TemplateGuide({params}:{params:Promise<{id:string}>}){
 ...
 const [{data:template},{data:version},{data:hasLaunchPad}]=await Promise.all([
  ...
  supabase.rpc('has_template_workspace_access',{p_site_id:id,p_template_key:'rava-service-living-system'}),
 ])
 if(state?.current_template_id&&!template&&!hasLaunchPad)notFound()
 ...
 const templateName=template?(locale==='fa'?template.name_fa:template.name_en):hasLaunchPad?l('لانچ‌پد راوا','RAVA LaunchPad'):l('قالب فعلی سایت','Current site template')
 const description=template?(locale==='fa'?template.description_fa:template.description_en):hasLaunchPad?l('قالب خدماتی لانچ‌پد با مدیریت محتوای محدود، پیش‌نمایش واقعی و انتشار کنترل‌شده.','The LaunchPad service Template with constrained content management, real preview and controlled publishing.'):l('اطلاعات این قالب هنوز تکمیل نشده است.','This template description has not been completed yet.')
  ...
  <div className={styles.shell}>
   ...
   <div className={styles.layout}>
    ...
    <article className={styles.content}>
     ...
     <header className={styles.hero} id="overview"><span>{l('قالب فعال','Active template')} · {l('نسخه','Version')} {version?.version??'—'}</span><h1>{templateName}</h1><p>{description}</p><div><b>{l('واکنش‌گرا','Responsive')}</b><b>{l('فارسی و انگلیسی','Persian and English')}</b><b>{l('آماده سئو','SEO ready')}</b><b>{template?.commercial_tier??(hasLaunchPad?l('اختصاصی','Entitled'):l('پایه','Core'))}</b></div></header>
