...
export async function createProductAction(_state:CommerceActionState,fd:FormData):Promise<CommerceActionState>{
  ...
  if(!uuid.test(siteId)||title.length<2||!/^[a-z0-9][a-z0-9-]{1,120}$/.test(slug)||!sku||!/^[A-Z]{3}$/.test(currency)||amount<0||stock<0)return{ok:false,message:'اطلاعات محصول معتبر نیست.',nonce:Date.now()}
  try{
    ...
    if(error)return failure(error,'commerce.product.create_failed',siteId,'ساخت محصول انجام نشد.',{title,slug,sku})
    refresh(siteId);const row=Array.isArray(data)?data[0]:data
    return{ok:true,message:`محصول ساخته شد${row&&typeof row==='object'&&'product_id'in row?` · ${String(row.product_id)}`:''}.`,nonce:Date.now()}
  }catch(error){return failure(error,'commerce.product.create_failed',siteId,'ساخت محصول انجام نشد.',{title,slug,sku})}
}
...
export async function adjustInventoryAction(_state:CommerceActionState,fd:FormData):Promise<CommerceActionState>{
  ...
  if(!uuid.test(siteId)||!uuid.test(variantId)||!uuid.test(locationId)||delta===0)return{ok:false,message:'اطلاعات تغییر موجودی معتبر نیست.',nonce:Date.now()}
  try{
    ...
    if(error)return failure(error,'commerce.inventory.adjust_failed',siteId,'تغییر موجودی انجام نشد.',{variantId,locationId,delta})
    refresh(siteId);const row=Array.isArray(data)?data[0]:data
    return{ok:true,message:`موجودی به‌روزرسانی شد${row&&typeof row==='object'&&'available'in row?` · قابل فروش: ${String(row.available)}`:''}.`,nonce:Date.now()}
  }catch(error){return failure(error,'commerce.inventory.adjust_failed',siteId,'تغییر موجودی انجام نشد.',{variantId,locationId,delta})}
}
...
export async function createDraftOrderAction(_state:CommerceActionState,fd:FormData):Promise<CommerceActionState>{
  ...
  if(!uuid.test(siteId)||orderNumber.length<3||!/^[A-Z]{3}$/.test(currency)||!uuid.test(variantId))return{ok:false,message:'اطلاعات سفارش معتبر نیست.',nonce:Date.now()}
  try{
    ...
    if(error)return failure(error,'commerce.order.create_failed',siteId,'ساخت سفارش انجام نشد.',{orderNumber,variantId,quantity})
    refresh(siteId);return{ok:true,message:`سفارش Draft ساخته شد${typeof data==='string'?` · ${data}`:''}.`,nonce:Date.now()}
  }catch(error){return failure(error,'commerce.order.create_failed',siteId,'ساخت سفارش انجام نشد.',{orderNumber,variantId,quantity})}
}
