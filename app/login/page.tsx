...
export default function LoginPage() {
  ...
  function validateBeforeSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    else if (password.length < 8 || password.length > 128) message = 'رمز عبور باید بین ۸ تا ۱۲۸ کاراکتر باشد.'
    else if (/\p{Cc}/u.test(email) || /\p{Cc}/u.test(password)) message = 'ورودی نامعتبر است.'
    else if (!captcha) message = 'تأیید امنیتی را کامل کنید.'
        ...
        <form action={action} className="auth-form" onSubmit={validateBeforeSubmit} noValidate>
          ...
          {siteKey ? (
            <div className="auth-captcha-wrap">
              <div className="cf-turnstile" data-sitekey={siteKey} data-action="admin-login" data-theme="dark" data-language="fa" />
            </div>
          ) : (
            <p className="auth-error" role="alert">کلید امنیتی CAPTCHA هنوز روی محیط سایت تنظیم نشده است.</p>
          )}
          ...
          <button type="submit" disabled={pending || !siteKey}>{pending ? 'در حال بررسی و ورود…' : 'ورود امن'}</button>
        </form>
